 ...
 * 
 * @param position “Camera”构造函数中的“position”参数表示相机在 3D 空间中的初始位置。它是“vec3”类型，通常包含三个浮点值，表示相机位置的 x、y 和 z
 * 坐标。在提供的代码片段中，
 * @param up “Camera”构造函数中的“up”参数表示相机局部坐标系中的向上方向。它通常用于定义相机的方向。在这种情况下，“up”的默认值是指向正 y 方向的向量（0.0f
 * @param yaw “Camera”构造函数中的“yaw”参数表示相机的水平旋转角度（以度为单位）。它用于确定相机沿水平轴朝向的方向。
 * @param pitch
 ...
 * 
 * @param direction “direction”参数是一个包含 6 个元素的数组，表示 3D 空间中的移动方向。每个元素对应一个特定的方向：
 * @param deltaTime `deltaTime` 参数表示当前帧与前一帧之间的时间差。它通常用于游戏开发，以确保无论帧速率如何，移动都能流畅一致。
 */
